...
impl ArciumCommand {
    ...
    async fn init(&self, name: &str) -> Result<()> {
        println!("{} Initializing Arcium Project: {}", "→".bright_cyan(), name);
        ...
        if self.check_arcium_cli().await {
            ...
            // This ensures the command works even without the binary
             ...
             println!("  {} Basic structure created. Install Arcium CLI for full features.", "✓".bright_green());
        }
    ...
    async fn run_node(&self, image: &str, detach: bool) -> Result<()> {
        ...
        if Command::new("docker").arg("--version").output().is_err() {
            println!("{} Docker is not installed or not in PATH.", "✗".bright_red());
            println!("  Arcium nodes require Docker to run.");
            return Ok(());
        }
        
        println!("  Image: {}", image.bright_blue());
        println!("  Mode: {}", if detach { "Background" } else { "Foreground" });
        println!();
        ...
        match cmd.spawn() {
            ...
            Err(e) => {
                println!("{} Failed to start Docker container: {}", "✗".bright_red(), e);
                println!();
    ...
    async fn stop_node(&self) -> Result<()> {
        ...
        match status {
            ...
            Err(_) => println!("{} Failed to stop node (is it running?)", "⚠".bright_yellow()),
        }
