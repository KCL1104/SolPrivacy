...
use light_client::{
    indexer::{
        GetCompressedTokenAccountsByOwnerOrDelegateOptions, Indexer, IndexerRpcConfig, RetryConfig,
    },
...
impl LightCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            ...
            LightAction::Accounts { owner, rpc_url } => {
                self.query_accounts(owner, rpc_url.clone()).await
            }
            LightAction::Balance {
                owner,
                mint,
                ...
                self.query_balance(owner, mint.clone(), rpc_url.clone())
                    .await
            }
    ...
    fn run_light_command(&self, args: &[&str]) -> Result<()> {
        ...
        if !output.status.success() {
            return Err(crate::error::SolPrivacyError::CommandFailed(
                "light command failed".to_string(),
            ));
        }
        Ok(())
    }
        ...
        if show {
            if let Ok(config) = AppConfig::load() {
                if let Some(ref photon_url) = config.rpc.photon_url {
                    ...
                    println!(
                        "  {} Photon RPC URL: {} (using default RPC)",
                        "ℹ".bright_blue(),
                        config.get_rpc_url()
                    );
                }
                ...
                println!(
                    "  {} Photon RPC URL: http://127.0.0.1:8899 (default)",
                    "ℹ".bright_blue()
                );
            }
            return Ok(());
    ...
    async fn query_accounts(&self, owner: &str, rpc_url: Option<String>) -> Result<()> {
        println!(
            "{} Query Compressed Accounts (Native SDK)",
            "→".bright_cyan()
        );
        println!("{}", "─".repeat(50).bright_black());
        println!();
        ...
        match rpc
            .get_compressed_accounts_by_owner(&owner_pubkey, None, Some(indexer_config))
            .await
        {
            Ok(response) => {
                ...
                if accounts.is_empty() {
                    ...
                    for (i, account) in accounts.iter().enumerate() {
                        ...
                        println!(
                            "    Hash: {}",
                            bs58::encode(&account.hash).into_string().bright_black()
                        );
    ...
    async fn query_balance(
        ...
        println!(
            "{} Query Compressed Token Balance (Native SDK)",
            "→".bright_cyan()
        );
        println!("{}", "─".repeat(50).bright_black());
        println!();
        ...
        println!(
            "  {} Querying compressed token balances...",
            "→".bright_cyan()
        );
        ...
        let options =
            mint_pubkey.map(|m| GetCompressedTokenAccountsByOwnerOrDelegateOptions::new(Some(m)));

        match rpc
            .get_compressed_token_balances_by_owner_v2(&owner_pubkey, options, Some(indexer_config))
            .await
        {
            Ok(response) => {
                ...
                if balances.is_empty() {
                    ...
                    for balance in &balances {
                        ...
                        println!(
                            "    Balance: {}",
                            balance.balance.to_string().bright_green()
                        );
        ...
        match rpc
            .get_compressed_balance_by_owner(&owner_pubkey, Some(indexer_config))
            .await
        {
            Ok(response) => {
    ...
    async fn compare_costs(&self, count: u64, size: u64) -> Result<()> {
        println!(
            "{} Cost Comparison: Regular vs Compressed",
            "→".bright_cyan()
        );
        println!("{}", "─".repeat(50).bright_black());
        println!();
        ...
        println!(
            "    Per account: {} lamports ({:.6} SOL)",
            regular_rent_per_account,
            regular_rent_per_account as f64 / 1e9
        );
        println!(
            "    Total ({} accounts): {} lamports ({:.6} SOL)",
            count, regular_total, regular_sol
        );
        println!();
        ...
        println!(
            "    Per account: ~{} lamports ({:.6} SOL)",
            compressed_per_account,
            compressed_per_account as f64 / 1e9
        );
        println!(
            "    Total ({} accounts): ~{} lamports ({:.6} SOL)",
            count, compressed_total, compressed_sol
        );
        println!();

        println!("  {}:", "Savings".bright_green());
        println!(
            "    Total saved: {} lamports ({:.6} SOL)",
            savings, savings_sol
        );
        ...
        if compressed_total > 0 {
            ...
            println!(
                "    Regular accounts cost {:.0}x more than compressed",
                multiplier
            );
        }
        println!();
    ...
    async fn show_workflow(&self) -> Result<()> {
        ...
        println!(
            "    solprivacy light config --rpc-url https://devnet.helius-rpc.com/?api-key=YOUR_KEY"
        );
        println!();
        ...
        println!(
            "    • {} - Data stored off-chain, only hash on-chain",
            "Compressed Accounts".bright_white()
        );
        println!(
            "    • {} - Merkle tree storing account state",
            "State Tree".bright_white()
        );
        println!(
            "    • {} - ZK proof of account validity",
            "Validity Proof".bright_white()
        );
        println!(
            "    • {} - RPC with indexer support",
            "Photon RPC".bright_white()
        );
    ...
    async fn setup(&self) -> Result<()> {
        ...
        if self.is_light_installed() {
            let version = Command::new("light")
                .arg("--version")
                .output()
                .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
                .unwrap_or_default();
            println!(
                "{} Light CLI already installed: {}",
                "✓".bright_green(),
                version
            );
            println!();
        ...
        println!(
            "    {}",
            "npm i -g @lightprotocol/zk-compression-cli".bright_cyan()
        );
        println!();
    ...
    async fn create_mint(&self, decimals: u8) -> Result<()> {
        ...
        println!();
        println!(
            "{} Running: light create-mint --decimals {}",
            "→".bright_cyan(),
            decimals
        );
        println!();
    ...
    async fn show_info(&self) -> Result<()> {
        ...
        println!(
            ...
            "State Trees".bright_cyan()
        );
        println!(
            "    • {} - Off-chain data indexing",
            "Photon Indexer".bright_cyan()
        );
        println!(
            "    • {} - Proves account validity",
            "ZK Proofs".bright_cyan()
        );
        println!();
