...
impl KeygenCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.key_type {
            KeyType::Elgamal { output, encrypt, force } => {
                self.generate_elgamal_keypair(output, *encrypt, *force, "elgamal")?;
            }
            KeyType::Auditor { output, encrypt, force } => {
                self.generate_elgamal_keypair(output, *encrypt, *force, "auditor")?;
            }
            KeyType::Recover { output, encrypt, force, index } => {
                self.recover_keypair(output, *encrypt, *force, *index).await?;
            }
        }
    ...
    fn generate_elgamal_keypair(&self, output: &str, encrypt: bool, force: bool, key_type: &str) -> Result<()> {
        ...
        let public_b64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, public_bytes);
        ...
        if encrypt {
            ...
            self.save_encrypted(output_path, &secret_bytes, &public_b64, key_type, &password)?;
             ...
             println!("{} {}", "WARN: Saving key in plaintext.".bright_yellow(), "Use --encrypt for production.".bright_yellow());
             
             let kp = ElGamalKeypairJson {
                public_key: public_b64.clone(),
                secret_key: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, secret_bytes),
    ...
    async fn recover_keypair(&self, output: &str, encrypt: bool, force: bool, index: u32) -> Result<()> {
        let output_path = Path::new(output);
        ...
        let public_point = &secret_scalar * RISTRETTO_BASEPOINT_TABLE;
        let public_b64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, public_point.compress().as_bytes());

        if encrypt {
             let password = rpassword::prompt_password("Enter password to encrypt key: ")?;
             let confirm = rpassword::prompt_password("Confirm password: ")?;
             if password != confirm { return Err(SolPrivacyError::Input("Mismatch".into())); }
             
             self.save_encrypted(output_path, &final_secret, &public_b64, "elgamal", &password)?;
        } else {
             let kp = ElGamalKeypairJson {
                public_key: public_b64.clone(),
                secret_key: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, final_secret),
    ...
    fn save_encrypted(&self, path: &Path, secret: &[u8; 32], public_key: &str, key_type: &str, password: &str) -> Result<()> {
        ...
        let encrypted_kp = EncryptedKeypairJson {
            ...
            encrypted_secret: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, ciphertext),
    ...
    fn print_success(&self, path: &str, public_key: &str, encrypted: bool) {
        ...
        if encrypted {
            println!("  Security: {}", "ENCRYPTED (Argon2id + ChaCha20)".bright_green());
        } else {
            println!("  Security: {}", "PLAINTEXT (Unsafe for production)".bright_red());
        }
    }
}
