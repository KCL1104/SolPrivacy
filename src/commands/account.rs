...
impl AccountCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            AccountAction::Create { mint, owner, keypair } => {
                self.create_account(mint, owner.as_deref(), keypair.as_deref()).await
            }
            AccountAction::Info { account } => {
                self.show_info(account).await
            }
    ...
    async fn create_account(
        ...
        println!("      solprivacy transfer --mint {} --to {} --amount 100", mint, owner_pubkey);
    ...
    async fn show_info(&self, account: &str) -> Result<()> {
        ...
        match client.get_account(&account_pubkey) {
            Ok(acc) => {
                let is_token_2022 = acc.owner.to_string() == "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb";
                
                println!("  {}:", "Account Info".bright_white());
                println!("  ├─ Address: {}", account_pubkey);
                println!("  ├─ Program: {}", if is_token_2022 { "Token-2022" } else { "Unknown" });
                ...
                let keypair_path = match keypair_path {
                    ...
                    None => {
                        ...
                        match default {
                            Some(p) if std::path::Path::new(&p).exists() => p,
                            _ => {
                                println!("{} Wallet address or keypair required!", "✗".bright_red());
                                return Ok(());
                            }
                        }
                    }
