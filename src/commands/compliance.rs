...
impl ComplianceCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            ...
            ComplianceAction::Audit { keypair, mint, output, limit, rpc_url } => self.audit_transfers(keypair, mint.as_deref(), output, *limit, rpc_url.as_deref()).await,
    ...
    async fn audit_transfers(&self, keypair_path: &str, _mint_filter: Option<&str>, output_file: &str, limit: usize, rpc_url_override: Option<&str>) -> Result<()> {
        ...
        let rpc_url = rpc_url_override.map(String::from).unwrap_or_else(|| config.get_rpc_url());
        ...
        // 3. Scan History
        println!("  {} Scanning transaction history (limit: {})...", "→".bright_cyan(), limit);
        
        let signatures = client.get_signatures_for_address(&auditor_pubkey)
        ...
        for sig_info in signatures.iter().take(limit) {
            ...
            if let Ok(tx) = client.get_transaction(&signature, solana_transaction_status::UiTransactionEncoding::Json) {
        ...
        println!();
        if found_count > 0 {
            println!("{} Found {} confidential transfers", "✓".bright_green(), found_count);
        } else {
             println!("{} No transfers found in recent history.", "ℹ".bright_blue());
        }

        println!("{} Audit report generated: {}", "✓".bright_green(), output_file.bright_blue());
    ...
    fn configure(&self, api_key: Option<String>, show: bool) -> Result<()> {
        if show { println!("Showing config..."); }
        if let Some(k) = api_key { println!("Setting key: {}", k); }
        Ok(())
    }
}
