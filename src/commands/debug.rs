...
impl DebugCommand {
    ...
    fn analyze_for_confidential_transfer(
        ...
        if let Some(ref meta) = tx.transaction.meta {
            ...
            if let solana_transaction_status::option_serializer::OptionSerializer::Some(log_vec) =
                logs
            {
                for log in log_vec {
                    if log.contains(TOKEN_2022_PROGRAM_ID)
                        || log.contains("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb")
                    {
                        has_token_2022 = true;
                    }
        ...
        if has_token_2022 {
            ...
            if has_confidential {
                ...
                if self.auditor_key.is_some() {
                    ...
                    println!(
                        "  └─ {} Provide --auditor-key to decrypt amounts",
                        "ℹ".bright_blue()
                    );
                }
            } else {
                println!(
                    "  └─ {} Token-2022 transaction (non-confidential)",
                    "ℹ".bright_blue()
                );
            }
        } else {
    ...
    fn decrypt_with_auditor_key(
        ...
        println!(
            "  ├─ Auditor Key Loaded: {}...",
            &keypair.public_key[..16.min(keypair.public_key.len())]
        );
        ...
        if secret_bytes.len() != 32 {
            println!(
                "  └─ {} Secret key invalid length: {} (expected 32)",
                "✗".bright_red(),
                secret_bytes.len()
            );
            return Ok(());
        ...
        if public_bytes.len() != 32 {
            println!(
                "  └─ {} Public key invalid length: {} (expected 32)",
                "✗".bright_red(),
                public_bytes.len()
            );
            return Ok(());
        ...
        if computed_compressed == stored_compressed {
            println!(
                "  ├─ {} Keypair is valid ElGamal on Ristretto255",
                "✓".bright_green()
            );
            println!(
                "  ├─ {} Public key matches secret key (P = s·G)",
                "✓".bright_green()
            );
        } else {
    ...
    fn print_usage(&self) {
        ...
        println!(
            "    {} Inspect a confidential transfer transaction",
            "•".bright_cyan()
        );
        println!("      solprivacy debug --decrypt --tx 5wH...xyz");
        println!();
        println!(
            "    {} Decrypt amounts using auditor key",
            "•".bright_cyan()
        );
        ...
        if let Some(ref tx_sig) = self.tx {
            ...
            match client.get_transaction(&signature, encoding) {
                Ok(tx) => {
                    ...
                    if self.raw {
                        let json =
                            serde_json::to_string_pretty(&tx).map_err(SolPrivacyError::Serde)?;
                        println!("\n{}", json);
                    }
                }
                Err(e) => {
