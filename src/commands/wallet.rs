...
impl WalletCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            ...
            WalletAction::Import { name, private_key, keypair } => {
            ...
            WalletAction::Airdrop { amount, wallet } => self.request_airdrop(*amount, wallet.as_deref()).await,
    ...
    fn import_wallet(&self, name: &str, private_key: Option<&str>, keypair_path: Option<&str>) -> Result<()> {
        ...
        let keypair = if let Some(pk) = private_key {
            ...
            if bytes.len() == 32 {
                let secret_array: [u8; 32] = bytes.try_into()
                ...
                let secret_array: [u8; 32] = bytes[..32].try_into()
                    .map_err(|_| SolPrivacyError::Crypto("Invalid key length".to_string()))?;
                Keypair::new_from_array(secret_array)
            } else {
                return Err(SolPrivacyError::Crypto(format!("Invalid key length: {} bytes (expected 32 or 64)", bytes.len())));
            }
    ...
    async fn show_balance(&self, wallet: Option<&str>) -> Result<()> {
        ...
        match client.get_token_accounts_by_owner(&pubkey, TokenAccountsFilter::ProgramId(token_2022_program_id())) {
            Ok(accounts) => {
                if accounts.is_empty() {
                    println!("  └─ No Token-2022 accounts");
                } else {
                    for (i, acc) in accounts.iter().enumerate() {
                        let prefix = if i == accounts.len() - 1 { "└─" } else { "├─" };
                        if let Ok(balance) = client.get_token_account_balance(&Pubkey::from_str(&acc.pubkey).unwrap()) {
                            println!("  {} {} ({})", prefix, balance.ui_amount_string, &acc.pubkey[..16]);
                        }
                    }
                }
            }
                ...
                match client.confirm_transaction(&signature) {
                    Ok(true) => {
                        ...
                        if let Ok(balance) = client.get_balance(&pubkey) {
                            let sol = balance as f64 / LAMPORTS_PER_SOL as f64;
                            println!();
                            println!("  New Balance: {} SOL", format!("{:.9}", sol).bright_green());
                        }
                    }
                    _ => {
                        println!("{} Confirmation pending - check balance later", "⚠".bright_yellow());
                    }
                }
            }
            Err(e) => {
    ...
    fn show_address(&self, name: Option<&str>) -> Result<()> {
        ...
        if !wallet_path.exists() {
            ...
            println!("  Create one with: solprivacy wallet new --name {}", wallet_name);
            return Ok(());
        }
            ...
            None => {
                ...
                if wallet_path.exists() {
                    ...
                    if let Some(path) = solana_default {
                        ...
                        Err(SolPrivacyError::Other("No default wallet. Create one with: solprivacy wallet new".to_string()))
                    }
                }
            }
        }
