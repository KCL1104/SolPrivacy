...
impl HeliusCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            ...
            HeliusAction::Fees {
                ...
                self.get_priority_fees(transaction.clone(), accounts.clone(), level)
                    .await
            }
            ...
            HeliusAction::Send {
                ...
                self.send_transaction(transaction, *skip_preflight, *auto_fees)
                    .await
            }
                        ...
                        Ok(data) => {
                            ...
                            if let Some(result) = data.get("result") {
                                if let Some(estimate) = result.get("priorityFeeEstimate") {
                                    ...
                                    println!(
                                        "    {} microlamports/CU",
                                        format!("{:.0}", estimate.as_f64().unwrap_or(0.0))
                                            .bright_green()
                                    );
                                    println!();
                                }
                                ...
                                if let Some(levels) = result.get("priorityFeeLevels") {
                                    ...
                                    if let Some(min) = levels.get("min") {
                                        println!(
                                            "    Min:      {:>10.0} microlamports/CU",
                                            min.as_f64().unwrap_or(0.0)
                                        );
                                    }
                                    if let Some(low) = levels.get("low") {
                                        println!(
                                            "    Low:      {:>10.0} microlamports/CU",
                                            low.as_f64().unwrap_or(0.0)
                                        );
                                    }
                                    if let Some(medium) = levels.get("medium") {
                                        println!(
                                            "    Medium:   {:>10.0} microlamports/CU {}",
                                    ...
                                    if let Some(high) = levels.get("high") {
                                        println!(
                                            "    High:     {:>10.0} microlamports/CU",
                                            high.as_f64().unwrap_or(0.0)
                                        );
                                    }
                                    if let Some(very_high) = levels.get("veryHigh") {
                                        println!(
                                            "    VeryHigh: {:>10.0} microlamports/CU",
                                            very_high.as_f64().unwrap_or(0.0)
                                        );
                                    }
                                    if let Some(unsafe_max) = levels.get("unsafeMax") {
                                        println!(
                                            "    UnsafeMax:{:>10.0} microlamports/CU {}",
    ...
    async fn create_webhook(
        ...
        println!(
            r#"    curl -X POST 'https://api.helius.xyz/v0/webhooks?api-key=YOUR_KEY' \
    ...
    async fn delete_webhook(&self, id: &str) -> Result<()> {
        ...
        println!("  {}:", "cURL Example".bright_white());
        println!(
            "    curl -X DELETE 'https://api.helius.xyz/v0/webhooks/{}?api-key=YOUR_KEY'",
            id
        );
    ...
    fn show_webhook_guide(&self) -> Result<()> {
        ...
        println!(
            "    • {} - Parsed, enriched transaction data",
            "enhanced".bright_cyan()
        );
    ...
    async fn send_transaction(
        ...
        println!(
            "  Transaction: {}...",
            &transaction[..32.min(transaction.len())]
        );
        ...
        println!(
            r#"    import {{ Helius }} from 'helius-sdk';
    ...
    fn show_info(&self) -> Result<()> {
        println!(
            "{} Helius API - Enhanced Solana Infrastructure",
            "→".bright_cyan()
        );
        println!("{}", "─".repeat(60).bright_black());
        println!();
