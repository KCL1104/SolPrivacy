...
impl ConfidentialCommand {
    pub async fn run(&self) -> Result<()> {
        match &self.action {
            ConfidentialAction::Configure { account, mint, elgamal_keypair, keypair, dry_run } => {
                self.configure_account(account, mint, elgamal_keypair.as_deref(), keypair.as_deref(), *dry_run).await
            }
            ConfidentialAction::Deposit { account, amount, mint, decimals, keypair, dry_run } => {
                self.deposit(account, *amount, mint, *decimals, keypair.as_deref(), *dry_run).await
            }
            ConfidentialAction::Apply { account, elgamal_keypair, keypair, dry_run } => {
                self.apply_pending(account, elgamal_keypair.as_deref(), keypair.as_deref(), *dry_run).await
            }
            ConfidentialAction::Transfer { mint, to, amount, elgamal_keypair, keypair, dry_run } => {
                self.transfer(mint, to, *amount, elgamal_keypair.as_deref(), keypair.as_deref(), *dry_run).await
            }
            ConfidentialAction::Withdraw { account, amount, mint, elgamal_keypair, keypair, dry_run } => {
                self.withdraw(account, *amount, mint, elgamal_keypair.as_deref(), keypair.as_deref(), *dry_run).await
            }
            ConfidentialAction::Balance { account, mint, elgamal_keypair, raw } => {
                self.show_balance(account, mint.as_deref(), elgamal_keypair.as_deref(), *raw).await
            }
            ConfidentialAction::Info { mint } => {
                self.show_info(mint).await
            }
            ConfidentialAction::Workflow => {
                self.show_workflow().await
            }
        }
    }
    ...
    async fn configure_account(
        ...
        println!("{} Configure Confidential Transfer Account", "→".bright_cyan());
        ...
        if let Some(path) = elgamal_keypair {
            match load_elgamal_pubkey(path) {
                Ok(pk) => {
                    println!("  {} ElGamal key loaded: {}...", "✓".bright_green(), &pk[..16.min(pk.len())]);
                }
                Err(e) => {
                    println!("  {} Failed to load ElGamal key: {}", "⚠".bright_yellow(), e);
                }
            }
        } else {
    ...
    async fn deposit(
        ...
        let decimals = match decimals {
            Some(d) => d,
            None if dry_run => {
                println!("  {} Using default decimals: 9 (provide --decimals for accuracy)", "ℹ".bright_blue());
                9
            }
            None => {
    ...
    async fn apply_pending(
        ...
        if let Some(path) = elgamal_keypair {
            match load_elgamal_pubkey(path) {
                Ok(pk) => {
                    println!("  {} ElGamal key: {}...", "✓".bright_green(), &pk[..16.min(pk.len())]);
                }
                Err(e) => {
    ...
    async fn transfer(
        ...
        println!("  ├─ Amount: {} (will be encrypted)", format!("{}", amount).bright_cyan());
        println!("  ├─ Mode: {}", "Confidential (ZK proofs required)".bright_green());
        println!("  └─ Network: {}", config.network);
        println!();
        ...
        if let Some(path) = elgamal_keypair {
            match load_elgamal_pubkey(path) {
                Ok(pk) => {
                    println!("  {} Sender ElGamal key: {}...", "✓".bright_green(), &pk[..16.min(pk.len())]);
                }
                Err(e) => {
                    println!("  {} ElGamal key error: {}", "⚠".bright_yellow(), e);
                }
    ...
    fn show_transfer_proofs(&self) {
        ...
        println!("  │");
        println!("  │  4. {} Zero-Balance Proof (if needed)", "→".bright_cyan());
    ...
    async fn withdraw(
        ...
        println!("  ├─ Account: {}...", &account[..16.min(account.len())]);
        println!("  ├─ Amount: {} (to public balance)", format!("{}", amount).bright_cyan());
        println!("  ├─ Mint: {}...", &mint[..16.min(mint.len())]);
        println!("  └─ Network: {}", config.network);
        println!();
            ...
            Ok(account_data) => {
                ...
                if account_data.data.len() > 165 {
                    ...
                    if raw {
                        ...
                        println!("  ├─ Pending balance: {}", "[encrypted ciphertext]".bright_yellow());
                        println!("  ├─ Available balance: {}", "[encrypted ciphertext]".bright_yellow());
                        println!("  │");
                        
                        if elgamal_keypair.is_some() {
                            println!("  └─ {} ElGamal key provided - decryption requires ZK SDK", "ℹ".bright_blue());
                        } else {
                            println!("  └─ {} Provide --elgamal-keypair to enable decryption", "ℹ".bright_blue());
                        }
                    }
                } else {
                    println!("  └─ Status: {} (account not configured)", "No CT Extension".bright_yellow());
                    println!();
                    println!("  Configure with:");
                    println!("    solprivacy confidential configure --account {} --mint <MINT>", account);
                }
            }
            Err(e) => {
    ...
    async fn show_info(&self, mint: &str) -> Result<()> {
        ...
        match client.get_account(&mint_pubkey) {
            Ok(account_data) => {
                ...
                println!("  ├─ Program: {}", if is_token_2022 { "Token-2022 ✓".bright_green() } else { "Unknown".bright_yellow() });
                ...
                if is_token_2022 && account_data.data.len() > 82 {
                    println!("  {} This mint likely supports confidential transfers", "✓".bright_green());
                    println!();
    ...
    async fn show_workflow(&self) -> Result<()> {
        ...
        println!("  │");
        println!("  ├─ {} Public balance is visible on-chain", "•".bright_cyan());
        println!("  ├─ {} Confidential balance is ElGamal encrypted", "•".bright_cyan());
        println!("  ├─ {} Pending balance: recently received (needs apply)", "•".bright_cyan());
        println!("  ├─ {} Available balance: ready for transfer/withdraw", "•".bright_cyan());
        println!("  └─ {} All confidential ops require ZK proofs", "•".bright_cyan());
        println!();
