...
impl QuickstartCommand {
    ...
    async fn show_list(&self) -> Result<()> {
        ...
        let tutorials = vec![
            ...
            (
                ...
                "Token-2022 private transactions",
                "10 min",
            ),
            (
                "compression",
            ...
            (
                "circuits",
                "Noir Circuits",
                "Build custom ZK proofs",
                "15 min",
            ),
            (
                "dapp",
                "Privacy dApp",
                "Build a complete privacy app",
                "20 min",
            ),
            (
                "compliance",
        ...
        for (cmd, name, desc, time) in tutorials {
            ...
            println!(
                "      {}",
                format!("solprivacy quickstart {}", cmd).bright_black()
            );
            println!();
        }
        ...
        println!(
            "    {} begin → wallet → confidential → compression → circuits",
            "1.".bright_cyan()
        );
        println!();
        println!(
            "  Run {} for a guided learning path",
            "solprivacy quickstart path".bright_cyan()
        );
    ...
    async fn tutorial_begin(&self) -> Result<()> {
        ...
        println!(
            "    {} Token-2022 Confidential Transfers (encrypted balances)",
            "•".bright_cyan()
        );
        println!(
            "    {} Light Protocol ZK Compression (400x cheaper accounts)",
            "•".bright_cyan()
        );
        println!(
            "    {} Noir Circuits (custom zero-knowledge proofs)",
            "•".bright_cyan()
        );
        println!(
            "    {} Range Protocol (compliance and risk assessment)",
            "•".bright_cyan()
        );
        println!();
    ...
    async fn tutorial_wallet(&self) -> Result<()> {
        ...
        self.print_tip(
            "Always keep your private keys secure. Never share them or commit them to git!",
        );
    ...
    async fn tutorial_confidential(&self) -> Result<()> {
        ...
        println!(
            "    {} Public balance → encrypted \"pending\" balance (deposit)",
            "1.".bright_cyan()
        );
        println!(
            "    {} Pending balance → usable \"available\" balance (apply)",
            "2.".bright_cyan()
        );
        println!(
            "    {} Transfer between encrypted balances (transfer)",
            "3.".bright_cyan()
        );
        println!(
            "    {} Encrypted balance → public balance (withdraw)",
            "4.".bright_cyan()
        );
        println!();
        ...
        println!();
        self.print_command(
            "solprivacy mint create --confidential --decimals 9 --name \"Private Token\"",
        );
        println!();
        ...
        println!();
        self.print_command(
            "solprivacy confidential transfer --mint <MINT> --to <RECIPIENT> --amount 500",
        );
        println!();
        ...
        self.print_next_step(
            "compression",
            "Learn about ZK compression with Light Protocol",
        );
    ...
    async fn tutorial_compression(&self) -> Result<()> {
        ...
        self.print_section("How It Works");
        println!(
            "    {} State is stored in Merkle trees instead of individual accounts",
            "•".bright_cyan()
        );
        println!(
            "    {} ZK proofs verify state transitions",
            "•".bright_cyan()
        );
        println!(
            "    {} Photon indexer tracks compressed state",
            "•".bright_cyan()
        );
        println!(
        ...
        println!();
        self.print_command(
            "solprivacy light config --url https://devnet.helius-rpc.com/?api-key=YOUR_KEY",
        );
        println!();
        ...
        self.print_tip(
            "Light Protocol is ideal for NFT collections, airdrops, and any app with many accounts",
        );
    ...
    async fn tutorial_circuits(&self) -> Result<()> {
        ...
        println!(
            "    {} - Prove inclusion in a set (whitelist/blacklist)",
            "merkle".bright_cyan()
        );
        println!(
            "    {} - Verify signatures anonymously",
            "signature".bright_cyan()
        );
        println!(
            "    {} - Prove value is in range (compliance)",
            "range".bright_cyan()
        );
        println!(
            "    {} - Prove sufficient balance (solvency)",
            "balance".bright_cyan()
        );
        println!(
        ...
        println!(
            "    {} - Complete private payment circuit",
        ...
        println!(
            "    {}",
            "fn main(secret: Field, pub commitment: Field) {".bright_black()
        );
        println!(
            "    {}",
            "    let computed = std::hash::poseidon::bn254::hash_1([secret]);".bright_black()
        );
        println!(
            "    {}",
            "    assert(computed == commitment);".bright_black()
        );
    ...
    async fn tutorial_dapp(&self) -> Result<()> {
        ...
        println!();
        self.print_command(
            "solprivacy mint create --confidential --decimals 6 --name \"PrivacyCoin\"",
        );
        println!();
        ...
        println!();
        self.print_command(
            "solprivacy zk init --circuit balance --output ./circuits/balance-proof",
        );
        println!();
        ...
        println!(
            "    {} If both pass, proceed with transfer",
            "3.".bright_black()
        );
        ...
        println!(
            "    {}",
            "npm install @solana/web3.js @solana/spl-token".bright_black()
        );
        println!(
            "    {}",
            "npm install @lightprotocol/stateless.js".bright_black()
        );
        ...
        println!(
            "    {}:",
            "User wants to send private payment".bright_white()
        );
        println!(
            "    {} Frontend generates ZK proof of sufficient balance",
            "1.".bright_cyan()
        );
        println!("    {} Compliance API checks recipient", "2.".bright_cyan());
        println!(
            "    {} Confidential transfer executes on-chain",
            "3.".bright_cyan()
        );
        println!(
            "    {} Recipient's balance updated (encrypted)",
            "4.".bright_cyan()
        );
        println!(
            "    {} Transaction logged (amounts hidden)",
            "5.".bright_cyan()
        );
        println!();
    ...
    async fn tutorial_compliance(&self) -> Result<()> {
        ...
        println!(
            "    {} Prevent interaction with sanctioned addresses",
            "•".bright_cyan()
        );
        ...
        println!();
        println!(
            "    {} {} - Clean address, no concerns",
            "0-2".bright_green(),
            "Low Risk".bright_green()
        );
        println!(
            "    {} {} - Some activity flags, monitor",
            "3-5".bright_yellow(),
            "Medium Risk".bright_yellow()
        );
        println!(
            "    {} {} - Significant concerns, caution",
            "6-8".bright_red(),
            "High Risk".bright_red()
        );
        println!(
            "    {} {} - Known malicious, block",
            "9-10".bright_red().bold(),
        ...
        println!(
            "    {}",
            "let sender_risk = check_risk(sender_address);".bright_black()
        );
        println!("    {}", "if sender_risk.score > 5 {".bright_black());
        println!(
            "    {}",
            "    return Err(\"Sender risk too high\");".bright_black()
        );
        println!("    {}", "}".bright_black());
        println!();
        ...
        println!(
            "    {}",
            "let recipient_risk = check_risk(recipient_address);".bright_black()
        );
        println!("    {}", "if recipient_risk.score > 7 {".bright_black());
        println!(
            "    {}",
            "    return Err(\"Recipient flagged\");".bright_black()
        );
        println!("    {}", "}".bright_black());
        println!();
        ...
        println!(
            "    {}",
            "execute_confidential_transfer(...);".bright_black()
        );
        println!();

        self.print_tip(
            "Privacy and compliance can coexist - check addresses without revealing amounts!",
        );
        ...
        println!(
            ...
            "•".bright_cyan(),
            "solprivacy examples list".bright_black()
        );
        println!(
            "    {} Read the docs: {}",
            "•".bright_cyan(),
            "solprivacy docs".bright_black()
        );
        println!(
            "    {} Join the community: {}",
            "•".bright_cyan(),
            "https://discord.gg/solprivacy".bright_black()
        );
    ...
    async fn show_learning_path(&self) -> Result<()> {
        ...
        for (num, cmd, name, desc, is_essential) in path {
            let marker = if is_essential {
                "●".bright_green()
            } else {
                "○".bright_black()
            };
            let essential = if is_essential {
                "(essential)"
            } else {
                "(advanced)"
            };
        ...
        println!(
            "  Start with: {}",
            "solprivacy quickstart begin".bright_cyan()
        );
    ...
    fn print_next_step(&self, cmd: &str, desc: &str) {
        ...
        println!(
            "    {} - {}",
            desc,
            format!("solprivacy quickstart {}", cmd).bright_cyan()
        );
    }
}
